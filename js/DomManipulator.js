...
const EXT_PREFIX = 'ext-EthereumAssistant';
...
class EthereumAssistant {
    ...
    setSearchAndReplaceSettings()
    {
        ...
        this.replacePatterns = [
            ...
            'data-address="$2"' +
            'class="ext-EthereumAssistant-link ext-EthereumAssistant-0xaddress" ' +
            'target="'+ this.target +'">' +
            '<div class="ext-EthereumAssistant-blockie" data-ether-address="$2" ></div> $2' +
            '</a>$3',

            // ENS Address Replace
            '$1<a title="See this address on the blockchain explorer" ' +
            'href="' + this.strBlockchainExplorer + '/$2" ' +
            'class="ext-EthereumAssistant-link" ' +
            ...
            'class="ext-EthereumAssistant-warning">$2</slot>$3'
        ];
    }
    ...
    setWarningSettings()
    {
        ...
        if(!this.ENSCompatiable){
            this.replacePatterns[1] = '$1<a title="Notification! We have spotted an ENS address, your current block explorer can\'t parse this address. Please choose a compatible block explorer." ' +
                'class="ext-EthereumAssistant-link ext-EthereumAssistant-warning" href="#">$2</a>$3';
        }
    }
        ...
        for(var i=0; i < objNode.childNodes.length; i++){
            ...
            if(objNode.childNodes[i].nodeType == 3){ // nodeType 3 = a text node
                ...
                if(this.isPatternMatched(childContent)) {
                    ...
                    replacement.setAttribute('class', 'ext-EthereumAssistant-temporary');
    ...
    tidyUpSlots()
    {
        var slots = document.querySelectorAll("slot.ext-EthereumAssistant-temporary");
    ...
    addBlockies()
    {
        var blockieDivs = document.querySelectorAll("div.ext-EthereumAssistant-blockie");
    ...
    {
        var objEtherAddresses = document.getElementsByClassName("ext-EthereumAssistant-link");
        for (var i = 0; i < objEtherAddresses.length; i++) {
            objEtherAddresses[i].classList.add("ext-EthereumAssistant-link-no_highlight");
            objEtherAddresses[i].classList.add("ext-EthereumAssistant-link-highlight");
        }
        return false;
    }
    ...
    addHighlightStyle()
    {
        ...
        for (var i = 0; i < objEtherAddresses.length; i++) {
            objEtherAddresses[i].classList.add("ext-EthereumAssistant-link-highlight");
            objEtherAddresses[i].classList.remove("ext-EthereumAssistant-link-no_highlight");
        }
        return false;
    }
    ...
    {
        var objNodes = document.getElementsByClassName("ext-EthereumAssistant-0xaddress");
        ...
        if (addressElement.children.length > 1 && addressElement.children[1].className === HOVER_POPUP_CLASS_NAME) {
            ...
            objBrowser.runtime.sendMessage({ func: "rpc_provider" }, (objResponse) => {
                ...
                web3.eth.getTransactionCount(str0xAddress, function(error, result) {
                    if(objHoverNodeContent.children[0].id == "ext-EthereumAssistant-fetching_data_"+uniqueAddressId) {
                    ...
                    if(error) {
                        ...
                        objHoverNodeContent.querySelector("#ext-EthereumAssistant-address_stats_hover_node_error_"+uniqueAddressId).style.display = "inline";
                        objHoverNodeContent.querySelector("#ext-EthereumAssistant-address_stats_hover_node_error_"+uniqueAddressId).innerText = "There were RPC errors";
                    } else {
                        objHoverNodeContent.querySelector("#ext-EthereumAssistant-address_stats_hover_node_ok_"+uniqueAddressId).style.display = "inline";
                        objHoverNodeContent.querySelector("#ext-EthereumAssistant-address_stats_hover_node_ok_"+uniqueAddressId).innerText = "RPC calls successful";
                        intTransactionCount = parseInt(result).toLocaleString();
                    }

                    var objTransactionCount = objHoverNodeContent.querySelector("#ext-EthereumAssistant-transactions_out_"+uniqueAddressId);
                ...
                web3.eth.getBalance(str0xAddress, function(error, result) {
                    ...
                    var objAddressBalance = objHoverNodeContent.querySelector("#ext-EthereumAssistant-address_balance_"+uniqueAddressId);
                ...
                web3.eth.getCode(str0xAddress, function(error, result) {
                    ...
                    var objContractAddress = objHoverNodeContent.querySelector("#ext-EthereumAssistant-contract_address_"+uniqueAddressId);

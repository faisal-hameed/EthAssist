...
let LOCALSTORAGE_IDENTIFER = {
    "DETAILS": "ext-EthereumAssistant-rpc_node_details",
    "ENDPOINT": "ext-EthereumAssistant-rpc_node"
}
...
class RpcNodeSelector
{
    ...
    static getCurrentNetworkDescription()
    {
        if(localStorage.getItem(LOCALSTORAGE_IDENTIFER.DETAILS)) {
            ...
            document.getElementById("ext-EthereumAssistant-rpcnode_current_details").innerText = `You are currently connected to: ${[objDetails.name, objDetails.type].join(" ")}`
        }
    }
    ...
    populateRpcNodeInput()
    {
        objBrowser.runtime.sendMessage({func: "rpc_provider"}, function(objResponse) {
            document.getElementById("ext-EthereumAssistant-rpcnode_modify_url").value = objResponse.resp;
        });
    ...
    {
        var objRpcValue = document.getElementById("ext-EthereumAssistant-rpcnode_modify_url");

        document.getElementById("ext-EthereumAssistant-rpcnode_connected_status").classList.add("hide-me");
        document.getElementById("ext-EthereumAssistant-rpcnode_details").classList.add("hide-me");

        var objRpcSuccessNode = document.getElementById("ext-EthereumAssistant-rpcnode_success");
    ...
    static rpcEndpointIsAvailable(strHttpProvider)
    {
        ...
        var objRpcErrorNode = document.getElementById("ext-EthereumAssistant-rpcnode_errors");
    ...
    static updateNodeDetails()
    {
        ...
        document.getElementById("ext-EthereumAssistant-rpcnode_connected_status").classList.remove("hide-me");
        document.getElementById("ext-EthereumAssistant-rpcnode_details").classList.remove("hide-me");

        var blConnected = objWeb3.isConnected();
        document.getElementById("ext-EthereumAssistant-rpcnode_connected_status").innerHTML = (blConnected ? "CONNECTED" : "DISCONNECTED") + "<br />";
        if(blConnected) {
            document.getElementById("ext-EthereumAssistant-rpcnode_details").innerHTML = `
...
(function() {
    ...
    var objForm = document.getElementById("ext-EthereumAssistant-rpcnode_modify_form");

...
function toggleBlockchainExplorer() {
    var objBlockchainExplorer = document.getElementById("ext-EthereumAssistant-choose_blockchain");
    localStorage.setItem("ext-EthereumAssistant-blockchain_explorer", objBlockchainExplorer.options[objBlockchainExplorer.selectedIndex].value);
    ...
    chrome.runtime.sendMessage({ func: "rpc_provider" }, (objResponse) => {
        ...
        if(intWeb3NetworkId !== intNetworkId) {
            document.querySelector(".ext-EthereumAssistant-note_network_diff").style.display = "block";
            document.querySelector(".ext-EthereumAssistant-note_network_diff").innerHTML = `
            ...
            document.querySelector(".ext-EthereumAssistant-note_network_diff").style.display = "none";
            document.querySelector(".ext-EthereumAssistant-note_network_diff").innerHTML = ``;
        }
    });
...
function refreshBlockchainExplorer() {
    var strBlockchainExplorer = localStorage.getItem("ext-EthereumAssistant-blockchain_explorer");

    if(strBlockchainExplorer === null) {
        document.getElementById("ext-EthereumAssistant-choose_blockchain").value = "https://etherscan.io/address";
    } else {
        document.getElementById("ext-EthereumAssistant-choose_blockchain").value = strBlockchainExplorer;
    }

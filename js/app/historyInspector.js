(function () {
    var objHistoryInspector = document.getElementById("ext-EthereumAssistant-history_inspect");
...
function doHistoryInspection() {
    ...
    objBrowser.history.search({text: "", maxResults: 500}, function (objHistoryItems) {
        ...
        var objDiv = document.getElementById("ext-EthereumAssistant-history_inspect_data");
        objDiv.innerHTML = "";
        ...
        for (var intIterator = 0; intIterator < objHistoryItems.length; intIterator++) {
            ...
            if (objUri.domain() === "harrydenley.com" && ["/EthereumAssistant/phishing.html", "/EthereumAssistant/phishing-segasec.html", "/EthereumAssistant/phishing-phishfort.html"].indexOf(objUri.path()) >= 0) {
                blRedirected = true;
                continue;
            }
            ...
            if (objBlacklistedDomains.domains.indexOf(objUri.domain()) >= 0) {
                strReportText += "<span class='ext-EthereumAssistant-note'>" + (new Date(objHistoryItems[intIterator].lastVisitTime).toUTCString()) + "</span>&nbsp;";
                //Did EAL redirect you away?
                if (blRedirected) {
                    strReportText += objUri.domain() + "<span class='ext-EthereumAssistant-note ext-EthereumAssistant-history_good'>EAL successfully redirected you away.</span>";
                } else {
                    strReportText += objUri.domain() + "<span class='ext-EthereumAssistant-note ext-EthereumAssistant-history_bad'>Domain is now blacklisted - but wasn't at the time.</span>";
                    ++intTotalWarnings;
                }
                strReportText += "<span class='ext-EthereumAssistant-note'><small>Visited "+ objHistoryItems[intIterator].visitCount +" times</small></span>";
                strReportText += "<br />";
            }
